...
#### b) 
The estimated survival curve does not appear to be headed towards zero as time goes to infinity. The curve seems to be levelling-off around .40 to .50 at around 1,500 days.
...
At t=1000, the mean residual lifetome for men is  1060.4 and for women, it is 1063.8.
```{r, echo=T}
#mrlife function
...
comp(fit)
```
Since the survival curve for men and women do not cross we can just look at the results in the first table. Results in the first table from the 6 tests all show a signfificant difference between the male and females curves at at the .05 level. Therefore we reject the null and conclude the alternative. We have enough evidence to conclude there is a difference between the survial curves for men and women at the .05 level. Based on the survival plots in part (e), males have higher survival probability.
...
fit.age <- survfit(Surv(wha$lenfol, wha$fstat) ~ wha$age2)
fit <- ten(fit.age)
comp(fit)
```
